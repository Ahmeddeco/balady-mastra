...
import { AddCattleAction } from "@/actions/cattle.action"
...
export default function AddCattleForm({ breeds, farms }: Props) {
	const [lastResult, action] = useActionState(AddCattleAction, undefined)
		...
		<Form id={form.id} action={action} onSubmit={form.onSubmit} className="space-y-6">
			<div className="grid lg:grid-cols-3 grid-cols-2 items-center gap-6">
				...
				<Field>
					...
					<Select key={fields.farmId.key} name={fields.farmId.name} defaultValue={fields.farmId.initialValue}>
						<SelectTrigger>
							<SelectValue />
						</SelectTrigger>
						<SelectContent>
				...
				<Field>
					...
					<Select key={fields.breedId.key} name={fields.breedId.name} defaultValue={fields.breedId.initialValue}>
						<SelectTrigger>
							<SelectValue />
						</SelectTrigger>
						<SelectContent>
				...
				<Field>
					...
					<Input
						...
						defaultValue={fields.liveWeight.initialValue}
					/>
			...
			{/* ------------------------------ SubmitButton ------------------------------ */}
			<SubmitButton text={"إحفظ بينات الحيوان"} />
		</Form>
	)
}
